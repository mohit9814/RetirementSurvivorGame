...
function applyRebalancing(state: GameState): GameState {
    const buckets = state.buckets; // Ref
    const strategy = state.config.rebalancingStrategy;
    if (strategy !== 'Tactical') return state;

    const tacticalYears = state.config.tacticalCashBufferYears || 5;
    ...
    // 1. Safety Refill
    if (buckets[0].balance < targetB1) {
        let deficit = targetB1 - buckets[0].balance;
        // From B2
        if (buckets[1].balance > 0) {
            const take = Math.min(buckets[1].balance, deficit);
            buckets[1].balance -= take;
            buckets[0].balance += take;
            deficit -= take;
        }
        // From B3
        if (deficit > 0 && buckets[2].balance > 0) {
            const take = Math.min(buckets[2].balance, deficit);
            buckets[2].balance -= take;
            buckets[0].balance += take;
        }
    }
    // 2. Deploy Idle Cash
    else if (buckets[0].balance > maxB1) {
        const excess = buckets[0].balance - targetB1;
        buckets[0].balance -= excess;
        buckets[1].balance += excess;
    }

    // 3. Profit Taking / Buy Low
    const totalWealth = buckets.reduce((s, b) => s + b.balance, 0);
    // B3 Logic
...
function debugRun() {
    ...
    const allocation = [0.05, 0.10, 0.85];
    const tacticalCashBufferYears = 2; // Standard
    ...
    const config: GameConfig = {
        ...
        inflationRate: 0.07,
        survivalYears: 50,
        enableTaxation: true, // Dummy
        rebalancingStrategy: 'Tactical',
        tacticalCashBufferYears: tacticalCashBufferYears,
        bucketConfigs: [
            { id: '1', name: 'Cash', type: 'Cash', allocation: allocation[0], expectedReturn: 0.045, volatility: 0.005 },
            { id: '2', name: 'Income', type: 'Income', allocation: allocation[1], expectedReturn: 0.109, volatility: 0.06 },
            { id: '3', name: 'Growth', type: 'Growth', allocation: allocation[2], expectedReturn: 0.193, volatility: 0.16 }
        ]
    };
    ...
    console.log(`DEBUG Success Rate: ${success}%`);
    if (wealths.length > 0) {
        console.log(`Median Wealth: ${(wealths.sort((a, b) => a - b)[Math.floor(wealths.length / 2)] / 10000000).toFixed(2)} Cr`);
    }
}
debugRun();
